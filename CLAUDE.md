...
### DM mode (v0.9+, slices 1-4 — foundation + bestiary CRUD + 5e.tools import + DM Roll surface)
...
- **Slice 4 (DM Roll surface)**: `DmRollView` renders each active monster as a card with the monster's actions, legendary actions, saves, and skills laid out as clickable buttons. Click composes through the same `compose()` as player Roll — `!attack "<action name>"` / `!save <ability>` / `!check <skill>` — and targets / modifiers / custom bonuses flow through the shared `RollSidePanel`. Each card also has a `↻ init add` button that bypasses the composer and emits `!init add 1d20 "<monster name>"` (init-add isn't one of compose's kinds and doesn't take targets/modifiers — separate code path). History labels prefix with the monster name (`Adult Lunar Dragon · Bite`) so it's distinguishable across multiple active monsters.
