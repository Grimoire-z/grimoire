...
- `src/views/BestiaryView.jsx` — DM mode's analogue of the vault: holds imported monster stat blocks, grouped into collapsible folder sections. Each card has an `active` checkbox (multi-select — multiple monsters can be active simultaneously) that will surface them on the DM Roll page once slice 4 lands. Per-card overflow menu has Rename / Duplicate / Delete + a folder picker; folder headers support inline rename + delete (folder delete moves contained monsters to Ungrouped). 5e.tools URL import lands in slice 3.
