...
- `src/views/BestiaryView.jsx` — DM mode's analogue of the vault: holds imported monster stat blocks, grouped into collapsible folder sections. Each card has an `active` checkbox (multi-select — multiple monsters can be active simultaneously) that will surface them on the DM Roll page once slice 4 lands. Card shows a compact two-line summary (size+type+alignment, then `CR · AC · HP`) when stat-block data is present; clicking the card body opens an in-place `StatBlockModal` with the full block (abilities grid, saves/skills/senses/languages/CR, traits/actions/legendary actions). Per-card overflow menu has Rename / Duplicate / Delete + a folder picker. Folder headers support inline rename + delete (folder delete moves contained monsters to Ungrouped). 5e.tools URL import in the Add-monster picker uses the main-process IPC handler.
...
- **Out-of-turn toggle (per-card)**: each monster card has an `☐ Out of turn` checkbox alongside the init-add button. When on, the monster's **attack** buttons flip from `!i a "<action>"` to **`!i aoo "<monster name>" "<action>"`** — Avrae's "attack out of turn" syntax for reactions, opportunity attacks, etc. (any case where a named combatant acts while it isn't its current init turn). The flag is per-monster and **ephemeral** (lives in DmRollView's local state, resets on tab/mode switch) since OOT is a moment-to-moment combat state rather than a stored property of the monster. Save/check buttons are intentionally **not** affected — Avrae's `!i save` / `!i check` don't have a clean standardized OOT form, so flipping them would invent syntax. The composer plumbing (`action.outOfTurn` + `action.combatantName`) is in place for future extension if a clear OOT pattern emerges for saves/checks. History labels append `(OOT)` when the flag is on so the strip stays disambiguated.
