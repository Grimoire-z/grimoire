...
- **Instance number (per-card dropdown)**: each card has a `# [—|1|2|…|10]` select in its header. When a number is set, it's suffixed onto the monster's name **wherever the combatant name appears in a command** — `Goblin` + 1 → `Goblin1`. That covers init-add (`!i madd "Goblin" -name "Goblin1"`) and the `!i offturn*` family (`!i offturnattack "Goblin1" "Scimitar" …`). The in-turn `!i a` / `!i s` / `!i c` commands don't include a combatant name (Avrae uses the current combatant from init), so the suffix doesn't affect those. History labels also get suffixed so the strip distinguishes between Goblin1's bite and Goblin2's bite. Ephemeral state, same lifetime as OOT + collapse. Useful pattern: duplicate a card N times in the Bestiary, assign each copy a unique instance number, then init-add them all to register Goblin1, Goblin2, Goblin3 as separate combatants in Avrae.
