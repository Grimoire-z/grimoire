...
- `src/components.jsx` — shared (Checkbox, ActionCard [with `tooltip`], ModifierRow, FieldLabel, SectionCard, D20Icon, PortraitDisplay, fileToPortraitDataUrl, `onActivate` keyboard helper, `Modal` shell, ConfirmDeleteModal). The five app modals all wrap `Modal` (accent/maxWidth/scroll/disabled/padded props)
...
### Import merge semantics (v0.10.2+)

`applyCharacterPatch` no longer replaces the `attacks` / `spells` arrays wholesale — a re-import (PDF, JSON paste, or DDB refresh) used to delete any custom actions the user had added by hand. Now `mergeEntriesById` (state.js):

- **updates** entries whose `id` matches an imported one (`{ ...existing, ...imported }`, so fields the import doesn't carry — a user-set `phrase`, the editor's `_key` — survive the refresh);
- **adds** imported entries with no existing match;
- **keeps** existing entries the import doesn't mention (the custom-action fix).

Spells merge per level via `mergeSpells`, with two extra rules: the existing `prepared` flag is **preserved on matches** (daily prep is a runtime decision the user toggles in the editor — re-importing must not trample it, matching the long-standing PDF-importer convention), and each touched level is re-sorted alphabetically so hand-added spells sit in reading order. Levels the patch doesn't mention are untouched.

Tradeoff (intended): something deleted on the DDB side lingers in Grimoire until removed manually in the editor. Matching is by `id` — attacks/spell ids are lowercased display names from the importers, so a renamed DDB action arrives as a *new* entry and its old version sticks around; that's the same lingering tradeoff, resolved by hand.

The Vault "create new character from import" path flows through the same helper, but merging over a blank character's empty lists is equivalent to the old replace, so creation behavior is unchanged. CharacterView's import buttons/copy now say "import & update" instead of "import & overwrite".
...
- A persisted setting (`settings.dmMode: boolean`) flips the whole app between **Player mode** (character vault + per-character surfaces) and **DM mode** (bestiary + monster-driven Roll). The toggle is a **`DM` button in the header**, grouped immediately to the left of the d20 Settings button (`Header` in `App.jsx`; the two buttons share a `flex gap-2` wrapper so they read as one right-hand cluster under the parent row's `justify-between`). The button is pressed-gold (`aria-pressed` + active styling) when DM mode is on and faded when off; its tooltip states the action ("Switch to DM mode" / "Switch to Player mode"). `App` passes `onToggleDmMode={() => setSettings(s => ({ ...s, dmMode: !s.dmMode }))}` into `Header`. (Was a "Mode" card at the bottom of Settings through v0.10.0 — moved to the header for one-click access since it's a frequent flip, not a rare deliberate action.) Switching is non-destructive: each mode's state stays intact in localStorage when you're in the other. The mode-flip safety effect (keyed on `settings.dmMode`) reroutes any now-invalid `mode` regardless of where the flip was triggered, so the header entry point needs no extra wiring.
...
v0.10.0 shipped a large multi-agent audit pass — 7 batches, documented in the architecture sections above (Data integrity, Correctness fixes, Build & Electron hardening, the dedup `Modal`/`dnd.js`/`useComposerEmit`/`signed` extractions, polish, Player Roll features, DM-mode features). Two pure-dedup micro-refactors were **deliberately deferred** (low value, real regression risk, not worth doing under release pressure): (1) lifting the duplicated card chrome — `CardActions`/`ActionMenuItem` + the rename inputs — shared between VaultView and BestiaryView (watch the `data-card-action`/stopPropagation differences that feed VaultView's click-to-enter skip); (2) a shared `DdbCaptureHelp` for the DDB capture instructions duplicated (and drifted) between VaultView's AddCharacterPicker and CharacterView's DdbImport. Also deferred: grouping the App roll-ephemerals into one session object (the audit itself judged it not worth it at the two-surface scale). None are correctness issues.
